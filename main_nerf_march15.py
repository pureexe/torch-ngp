...
if __name__ == '__main__':
    ...
    parser.add_argument('--og', action='store_true', help="use original nerf [Single Network]")
    ...
    if opt.test:
        ...
        if opt.gui:
            ...
            valid_dataset = NeRFDataset(opt.path, type='val', mode=opt.mode, downscale=opt.downscale, scale=opt.scale, preload=opt.preload) #should be val
            test_dataset = NeRFDataset(opt.path, type='test', mode=opt.mode, downscale=opt.downscale, scale=opt.scale, preload=opt.preload)
