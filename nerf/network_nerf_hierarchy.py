...
class NeRFNetwork(NeRFRenderer):
    def __init__(self, *args, **kwargs):
        ...
        print("============================")
        print("Network: NeRF (Original/Hierachy mode)")
    ...
    def build_sigma_net(self):
        ...
        self.sigma_net = nn.ModuleList([sigma_front0, sigma_back0, sigma_front1, sigma_back1])
    ...
    def forward(self, x, d, network_level=1, *args, **kwargs):
        x_ = self.encoder(x)
        h = self.sigma_net[(2*network_level) + 0](x_)
        h = self.sigma_net[(2*network_level) + 1](torch.cat([h,x_],dim=-1))
        sigma = torch.relu(h[...,:1])
        h_ = h[...,1:]
        d_ = self.encoder_dir(d)
        color = self.color_net[network_level](torch.cat([h_,d_],dim=-1))
        return sigma, color

        
    def density(self, x, *args,  **kwargs):
        x_ = self.encoder(x)
        h = self.sigma_net[2](x_)
        h = self.sigma_net[3](torch.cat([h,x_],dim=-1))
        sigma = torch.relu(h[...,:1])
        return sigma
